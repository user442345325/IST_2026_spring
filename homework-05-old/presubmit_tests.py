...
def test_QuadraticOracle():
    ...
    assert np.allclose(quadratic.grad(x), -b)
    assert np.allclose(quadratic.hess(x), A)
    assert isinstance(quadratic.grad(x), np.ndarray)
    assert isinstance(quadratic.hess(x), np.ndarray)

    # Check at point x = [1, 1, 1]
    x = np.ones(3)
    assert_almost_equal(quadratic.func(x), -4.5)
    assert np.allclose(quadratic.grad(x), x - b)
    assert np.allclose(quadratic.hess(x), A)
    assert isinstance(quadratic.grad(x), np.ndarray)
    assert isinstance(quadratic.hess(x), np.ndarray)
...
def check_log_reg(oracle_type, sparse=False):
    ...
    assert np.allclose(logreg.grad(x), [0, -0.25])
    assert np.allclose(logreg.hess(x), [[0.625, 0.0625], [0.0625, 0.625]])
    assert isinstance(logreg.grad(x), np.ndarray)
    assert isinstance(logreg.hess(x), np.ndarray)
...
def test_grad_finite_diff_2():
    ...
    assert isinstance(g, np.ndarray)
    assert np.allclose(g, [12.0, 6.0], atol=1e-4)
...
def test_hess_finite_diff_2():
    ...
    assert isinstance(H, np.ndarray)
    assert np.allclose(H, [[12.0, 0.], [0., 2.0]], atol=1e-3)
...
def check_equal_histories(history1, history2, atol=1e-3):
    if history1 is None or history2 is None:
        ...
        return
    assert 'func' in history1 and 'func' in history2
    assert np.allclose(history1['func'], history2['func'], atol=atol)
    assert 'grad_norm' in history1 and 'grad_norm' in history2
    assert np.allclose(history1['grad_norm'], history2['grad_norm'], atol=atol)
    assert 'time' in history1 and 'time' in history2
    assert len(history1['time']) == len(history2['time'])
    assert 'x' in history1 == 'x' in history2
    if 'x' in history1:
        assert np.allclose(history1['x'], history2['x'], atol=atol)
...
def check_prototype(method):
    ...
    assert len(mystdout.getvalue()) > 1
    sys.stdout = old_stdout
...
def check_one_ideal_step(method):
    ...
    assert np.allclose(x_star, [1.0, 2.0, 3.0])
    assert msg == 'success'
...
def test_gd_1d():
    ...
    assert np.allclose(x_star, [-0.4077], atol=1e-3)
    assert msg == 'success'
    ...
    [x_star, msg, history] = optimization.gradient_descent(oracle, x0,
                                                           ...
                                                           line_search_options={
                                                               'method': 'Constant',
                                                               'c': 1.0})
    assert np.allclose(x_star, [-0.4084371], atol=1e-2)
    assert msg == 'iterations_exceeded'
    assert history == None
...
def test_newton_1d():
    ...
    assert np.allclose(x_star, [-0.4077777], atol=1e-4)
    assert msg == 'success'
    check_equal_histories(history, TRUE_HISTORY)
...
def test_newton_fail():
    ...
    assert msg == 'computational_error'
    assert history == None
